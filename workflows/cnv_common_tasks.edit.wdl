...
task PreprocessIntervals {
    ...
    Int machine_mem_mb = select_first([mem_gb, 16]) * 1000 # EDITED
...
task FilterIntervals {
    ...
    Int machine_mem_mb = select_first([mem_gb, 512]) * 1000 # EDITED
    Int command_mem_mb = machine_mem_mb - 500
...
task PostprocessGermlineCNVCalls {
    ...
    Int machine_mem_mb = select_first([mem_gb, 128]) * 1000
    Int command_mem_mb = machine_mem_mb - 1000
    ...
    command <<<
        ...
        export HOME=$PWD
        export PYTENSOR_FLAGS="compiledir=$PWD/.pytensor"
        export MPLCONFIGDIR=$PWD/.mpl
        export XDG_CACHE_HOME=$PWD/.cache

        export OMP_NUM_THREADS=~{default=1 cpu} # EDITED
        export OPENBLAS_NUM_THREADS=~{default=1 cpu} # EDITED
        export MKL_NUM_THREADS=~{default=1 cpu} # EDITED
        ...
        #gatk --java-options "-Xmx~{command_mem_mb}m" PostprocessGermlineCNVCalls \ # EDITED
        gatk --java-options "-Xmx~{command_mem_mb}m -XX:ParallelGCThreads=1 -XX:ConcGCThreads=1" PostprocessGermlineCNVCalls \
            $calls_args \
            $model_args \
...
task CollectModelQualityMetrics {
    ...
    command <<<
        ...
        for index in ${!gcnv_model_tar_array[@]}; do
            ...
            NUM_POSITIVE_VALUES=$(awk '{ if (index($0, "@") == 0 && $0 !~ /^VALUE_/) {if ($1 > 0.0) {print $1} }}' MODEL_$index/mu_ard_u_interval__.tsv | wc -l)
