...
import "cnv_common_tasks.edit.wdl" as CNVTasks
...
workflow CNVGermlineCaseWorkflow {
    ...
    input {
      ...
      CollectCounts_outs CollectCounts # EDITED
    }
    ...
    scatter (sample_index in range(length(CollectCounts.entity_id))) { # EDITED
...
task DetermineGermlineContigPloidyCaseMode {
    ...
    command <<<
        ...
        export HOME=$PWD # EDITED
        export PYTENSOR_FLAGS="compiledir=$PWD/.pytensor" # EDITED
        export MPLCONFIGDIR=$PWD/.mpl # EDITED
        export XDG_CACHE_HOME=$PWD/.cache # EDITED
...
task GermlineCNVCallerCaseMode {
    ...
    command <<<
        ...
        # WORKAROUND: GATK v4.6.2.0 bug — retry seed can be negative, which numpy rejects.
        # Intercept 'python' calls and convert negative --random_seed to unsigned equivalent.
        cat > $PWD/python << 'PYWRAPPER'
#!/bin/bash
ARGS=()
for arg in "$@"; do
    if [[ "$arg" =~ ^--random_seed=(-[0-9]+)$ ]]; then
        neg="${BASH_REMATCH[1]}"
        pos=$(( (neg + 4294967296) % 4294967296 ))
        ARGS+=("--random_seed=$pos")
    else
        ARGS+=("$arg")
    fi
done
exec /opt/miniconda/envs/gatk/bin/python "${ARGS[@]}"
PYWRAPPER
        chmod +x $PWD/python
