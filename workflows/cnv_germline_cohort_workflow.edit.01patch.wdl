...
workflow CNVGermlineCohortWorkflow_01patch {
...
task GermlineCNVCallerCohortMode {
    ...
    command <<<
        ...
        gatk --java-options "-Xmx~{command_mem_mb}m -XX:ParallelGCThreads=1 -XX:ConcGCThreads=1"  GermlineCNVCaller \
            --run-mode COHORT \
            -L ~{intervals} \
            --input ~{sep=" --input " read_count_files} \
        ...
        CALL_TAR_LIST="gcnv_call_tars_shard_~{scatter_index}.list"
        : > $CALL_TAR_LIST
        CURRENT_SAMPLE=0
        ...
        while [ $CURRENT_SAMPLE -lt $NUM_SAMPLES ]; do
            ...
            TAR=~{cohort_entity_id}-gcnv-calls-shard-~{scatter_index}-sample-$CURRENT_SAMPLE_WITH_LEADING_ZEROS.tar.gz
            tar czf $TAR -C ~{output_dir_}/~{cohort_entity_id}-calls/SAMPLE_$CURRENT_SAMPLE .
            echo $TAR >> $CALL_TAR_LIST
    ...
    output {
        ...
        Array[File] gcnv_call_tars = read_lines("gcnv_call_tars_shard_~{scatter_index}.list")
...
task WritePathList {
    input {
        ...
        # Runtime parameters
        String sge_queue
        String docker = "betelgeuse:5000/python:latest"
